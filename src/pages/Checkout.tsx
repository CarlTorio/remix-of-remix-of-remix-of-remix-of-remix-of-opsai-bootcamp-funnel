...
const Checkout = () => {
  ...
  const handleUpload = async () => {
    ...
    if (!firstName || !lastName || !email || !phone) {
      toast({ title: "Please fill in all contact information", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      // 3. Save enrollment to DB (reference auto-generated by trigger)
      const batchMonth = new Date().toISOString().slice(0, 7); // e.g. "2026-04"
      const { data: enrollmentData, error: enrollError } = await supabase
        ...
        .insert({
          ...
          payment_method: paymentMethod,
          enrollment_reference: "TEMP", // will be overwritten by trigger
        })
        .select()
        .single();
      ...
      // 4. Send to Pancake via edge function
      const { error: pancakeError } = await supabase.functions.invoke(
        "send-enrollment-to-pancake",
        {
          body: {
            first_name: firstName,
            surname: lastName,
            phone,
            email,
            enrollment_reference: enrollmentData.enrollment_reference,
            payment_method: paymentMethod,
            batch_month: batchMonth,
          },
        }
      ...
      if (pancakeError) {
        console.error("Pancake sync error:", pancakeError);

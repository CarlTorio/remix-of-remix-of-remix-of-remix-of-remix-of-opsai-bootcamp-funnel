...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    class MouseClass {
      ...
      _onMouseMove: any; _onTouchStart: any; _onTouchMove: any; _onTouchEnd: any; _onDocumentLeave: any;
      constructor() {
      ...
      updateHoverState(cx: number, cy: number) { this.isHoverInside = this.isPointInside(cx, cy); return this.isHoverInside; }
    ...
    class ShaderPass {
      props: any; uniforms: any; scene: THREE.Scene | null = null; camera: THREE.Camera | null = null;
      material: any = null; geometry: any = null; plane: any = null;
    ...
    class Advection extends ShaderPass {
      ...
      update(p: any) { this.uniforms.dt.value = p.dt; this.line.visible = p.isBounce; this.uniforms.isBFECC.value = p.BFECC; super.update(); }
    }
    ...
    class Viscous extends ShaderPass {
      ...
      update(p: any) {
        ...
        for (let i = 0; i < p.iterations; i++) {
          ...
          this.uniforms.velocity_new.value = fbo_in.texture; this.props.output = fbo_out; this.uniforms.dt.value = p.dt; super.update();
        }
        return fbo_out;
      }
    }
    ...
    class Divergence extends ShaderPass {
      ...
      update(p: any) { this.uniforms.velocity.value = p.vel.texture; super.update(); }
    }
    ...
    class PressurePass extends ShaderPass {
      ...
      update(p: any) { this.uniforms.velocity.value = p.vel.texture; this.uniforms.pressure.value = p.pressure.texture; super.update(); }
    }
    ...
    class Simulation {
      ...
      createAllFBO() {
        ...
        const opts: any = { type, depthBuffer: false, stencilBuffer: false, minFilter: THREE.LinearFilter, magFilter: THREE.LinearFilter, wrapS: THREE.ClampToEdgeWrapping, wrapT: THREE.ClampToEdgeWrapping };
    ...
    class WebGLManager {
      props: any; output!: Output; autoDriver: AutoDriver; lastUserInteraction: number;
      running = false; _loop: any; _resize: any; _onVisibility: any;
      ...
      initManager() { this.props.$wrapper.prepend(Common.renderer!.domElement); this.output = new Output(); }
    ...
    if (sim) {
      ...
      if (resolution !== sim.options.resolution) sim.resize();
    }
